...
const NOTES: NoteDef[] = [
  { n: 1, title: "Category hero (SEO landing)", body: "H1 targets the category keyword using the agreed re-title — 'Oil Flooded Air Compressors' (keep 'oil injected screw' in the intro copy, buyers still search that term). This template serves all six Products-menu categories. Note the taxonomy crosses two axes: Stationary/Portable (configuration) and Oil Flooded/Oil Free (air type) — a model like the VOC 90 V appears under both without duplicate pages; the two taxonomies handle the cross-listing, with a faceting plugin driving the front-end filters (the core Query Loop block cannot facet on meta by itself)." },
  ...
  { n: 3, title: "Series listing with spec ranges", body: "Series rows show real kW/cfm ranges from the client's spec sheet, so buyers pick a family (44 in the catalogue) before a model (150). Data comes from the same ACF fields as the product pages — no duplicate content entry." },
