...
const NOTES: NoteDef[] = [
  ...
  { n: 5, title: "Specifications as ACF Groups, not a matrix", body: "ACF PRO BUILD SPEC. The Product custom post type stores four ACF Group fields: spec_performance, spec_drive, spec_physical and spec_controls. Each group contains named Number, Text or Select sub fields, so the editor sees an ordinary form and the block renders a simple two column label and value list. Values map directly to ProductList-Marketing.xlsx. An optional additional_specs repeater handles attributes that only some models carry, and an ACF Clone field reuses the same specification definitions across the Product post type. Register the specification section as a dynamic ACF Block with block.json and a PHP render template so Gutenberg shows a live preview. Use the block editor bundled with WordPress core rather than the standalone Gutenberg beta plugin." },
  ...
  { n: 8, title: "Related products by taxonomy", body: "Positioned below the quote form per client direction: the page drives to the quote request first, then offers sideways moves within the same series or air type. Use a core Query Loop filtered by the current Product taxonomies with one registered card template. New models appear automatically and editors do not curate this section by hand." },
  { n: 9, title: "Salesforce integrated quote form", body: "Request a Quote posts to Salesforce and redirects to a thank you page for conversion tracking. The current Product post supplies hidden model, part number and page fields from ACF. Use a block native forms plugin such as Gravity Forms, WS Form or Fluent Forms, or retain the existing Ninja Forms Salesforce integration to avoid remapping CRM fields. Place the form in a synced pattern and retest the thank you redirect, UTM values and GCLID after migration." },
